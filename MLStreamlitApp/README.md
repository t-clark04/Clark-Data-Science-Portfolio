...
1. Choose values for each of the predictors, as well as the type of classification model they'd like to use for prediction.
2. Tune the mode's hyperparameters, or allow the app to tune the hyperparameters for them.
3. Observe the outputted probability given the selected input values.
4. Evaluate the model's predictive power using the displayed metrics and visualizations.

Machine learning is the future! Try it out for yourself, and see how easy yet powerful machine learning can be!
...
### The "Easy" Way:
Simply click [here](https://clark-machine-learning.streamlit.app/). This link should bring up the deployed version of the app on the Streamlit website. No further steps necessary -- just choose your path and start exploring! The only downside is that you won't be able to see the underlying Python code if you so desired.

### The "Hard" Way:
You'll need to start by downloading the "MLStreamlitApp" folder from my data science portfolio repository. To do that, first go to [this link](https://download-directory.github.io/). It will open up a page that looks like this:
